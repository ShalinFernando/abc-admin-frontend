...
export default function TransactionManagementComponent() {
  ...
  const fetchData = async () => {
    try {
      const result = await getApi(apiCollection.transaction, null);
      setUserObj(result.data.result);
    } catch (errors) {
      console.log(errors);
    }
  };
  ...
  return (
    <LayoutComponent>
      <div>
        <Row gutter={[24, 24]}>
          <Col xs={24} sm={4} md={4}>
            <img src="./icons/item.svg" alt="title" style={{ width: 135 }} />
          </Col>
        ...
        <Divider />
        <TableComponent tableData={userObj} fetchData={fetchData} />
        <CreateTransaction
          visible={visible}
